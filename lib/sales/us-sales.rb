class UsSales
  ...
  def get_sales
    ...
    inbound = %w(AGENTDIRECT).map { |group|
      vici_stats.get_data 'http://5325844584:b3Z4RgkNG@unidial643.getcelerity.com/unidial/AST_CLOSERstats.php', in_post_hash, group
    }
    ...
    outbound = %w(1000 1001).map { |group|
      vici_stats.get_data 'http://5325844584:b3Z4RgkNG@unidial643.getcelerity.com/unidial/AST_VDADstats.php', out_post_hash, group
    }
    ...
    send_event('us-total-inbound-sales', {current: inbound_sales, last: @current_inbound_sales})
    @current_inbound_sales = inbound_sales

    send_event('us-total-inbound-calls', {current: inbound_calls, last: @current_inbound_calls})
    @current_inbound_calls = inbound_calls


    send_event('us-total-outbound-sales', {current: outbound_sales, last: @current_outbound_sales})
    @current_outbound_sales = outbound_sales

    send_event('us-total-outbound-calls', {current: outbound_calls, last: @current_outbound_calls})
    @current_outbound_calls = outbound_calls

    send_event('us-total-sales', {current: inbound_sales + outbound_sales + outbound_post_date + inbound_post_date, last: @current_total_sales})
    @current_total_sales = outbound_sales + inbound_sales

    send_event('us-total-post-date-sales', {current: outbound_post_date + inbound_post_date, last: @current_post_date_sales})
    @current_post_date_sales = outbound_post_date + inbound_post_date

    send_event('us-sales-graph', {points: week_sales_data_points})
