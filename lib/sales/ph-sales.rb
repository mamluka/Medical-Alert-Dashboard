class PhSales
  ...
  def get_sales
    ...
    send_event('ph-total-inbound-sales', {current: inbound_sales, last: @current_inbound_sales})
    @current_inbound_sales = inbound_sales

    send_event('ph-total-inbound-calls', {current: inbound_calls, last: @current_inbound_calls})
    @current_inbound_calls = inbound_calls


    send_event('ph-total-outbound-sales', {current: outbound_sales, last: @current_outbound_sales})
    @current_outbound_sales = outbound_sales

    send_event('ph-total-outbound-calls', {current: outbound_calls, last: @current_outbound_calls})
    @current_outbound_calls = outbound_calls

    send_event('ph-total-sales', {current: inbound_sales + outbound_sales + outbound_post_date + inbound_post_date, last: @current_total_sales})
    @current_total_sales = outbound_sales + inbound_sales

    send_event('ph-total-post-date-sales', {current: outbound_post_date + inbound_post_date, last: @current_post_date_sales})
    @current_post_date_sales = outbound_post_date + inbound_post_date

    send_event('ph-sales-graph', {points: week_sales_data_points})
